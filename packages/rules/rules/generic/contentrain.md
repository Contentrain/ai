...
> **WARNING — Dictionary saves are upsert.** When you call `contentrain_content_save` on a dictionary model, existing keys whose values differ from the new payload will be **overwritten**. Keys not present in the payload are preserved, but any key you include replaces the stored value.
>
> - **Always** call `contentrain_content_list` first to review existing keys and values before saving.
> - **Include all keys** you want to exist in a single save call. Sending partial key sets in multiple calls risks overwriting keys added by a previous call if the same key appears with a different value.
> - Treat dictionary save as "merge with overwrite on conflict" — not as "append only".
...
These two identifiers serve completely different purposes — do not confuse them.

| Aspect | Collection ID | Dictionary Key |
|--------|--------------|----------------|
| Format | Opaque 12-char hex (e.g., `a1b2c3d4e5f6`) | Semantic dot-separated address (e.g., `auth.login.button`) |
| Generated by | System (auto-generated on save) | Agent (you provide the key name) |
| Purpose | CRUD identifier for structured entries | i18n lookup address for flat key-value strings |
| Meaning | No semantic meaning — treat as opaque handle | Hierarchical and human-readable — encodes feature/domain |
| Used in | `contentrain_content_save(id: "...")`, `contentrain_content_delete(id: "...")` | i18n functions: `$t('auth.login.button')`, `t('errors.404.title')` |
| When to use | Repeating items with structured fields (team members, FAQs, products) | UI strings, labels, error messages — flat key-value, all string values, no field schema |
