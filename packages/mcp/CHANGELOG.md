...
- 93ab71d: Local provider: writes no longer trust the developer's checkout as the content truth.
...
- ae0b3ec: GitHub provider: `applyPlan` and `createBranch` take a full commit SHA as the base (compare-and-set writes).
...
- d064027: The registry description said 24 tools; there are 27
  ...
  `@contentrain/rules`'s `MCP_TOOLS`: the registry entry describes what _this_
...
- a9c8733: Allowlisted migration write path — consent, scope, and an audit trail in front of `applyPlan`
...
- 463297e: `PATH_PATTERNS.content.document` names the model it always required
...
- 86b32b5: `contentrain_bulk` takes `dry_run`
...
- 9d292de: Reconcile merges `.contentrain/config.json` by top-level key
  ...
  key, and only a key BOTH sides moved _differently_ becomes a question. The
...
- 487a061: Four fixes for models and content that predate the current rules, from issues #116, #120, #124 and #125.

  - **Legacy field names are grandfathered (#116).** `contentrain_model_save` enforced snake*case on every field of every save, so a model carrying one pre-rule `creativeWork` field was read-only through MCP — even a one-line `title_field` correction was refused. The existing model is now read first; a name it already has is kept and reported as a `schema_warning`, while a \_new* camelCase field is still rejected. `contentrain_validate` lists such fields as notices, so the two validators agree on what a valid model is.
