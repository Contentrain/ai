...
1. **Generates** the project in place, with no model:
   ...
   - **Studio:** the plan's Studio binding goes into `studio.json` when the starter reads it from there, in the shape Studio writes when it moves the site's media; an older starter gets it in `site.config.ts`.
