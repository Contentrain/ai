...
- c659098: `contentrainLoader` type-checks in Astro 6 and 7 projects with `strict` — `create-astro`'s default — or `exactOptionalPropertyTypes`. `ContentrainLoaderContext` typed `generateDigest` as taking `unknown`, narrower than Astro's `Record<string, unknown> | string`, so `defineCollection({ loader: contentrainLoader(…) })` failed `astro check` with TS2322. Its members now take Astro's own parameter types and every optional member admits `undefined`. No runtime change.
...
- adf26c1: `FormsClient` matches Studio's public forms contract; new `CommentsClient`
...
- d617dab: feat(sdk)!: CDN `document().all()` returns index entries, so reading `.body` no longer compiles
  ...
  The trap is that the bundled runtime's `all()` _does_ carry bodies. Code written
