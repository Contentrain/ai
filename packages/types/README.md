...
`RawIR` also carries five optional parts from the bridge rung:

- **`seo`** (`RawSeo`) holds which plugin serves the head (`serving`: `yoast` / `rank_math` / `aioseo` / `seopress` / `wordpress-core`; `seopress` may be absent from `providers` in an older export), each provider's status and settings, and each page's values keyed by `post:<id>` or `term:<taxonomy>:<id>` — plus `home`, one block per provider, for a home page that lists posts (a static front page is its page's `post:<id>`). `status: 'none'` is an answer, not a missing export: no SEO plugin, and WordPress core renders only the title. `resolved: true` marks values the running plugin rendered; `false` marks stored values and templates only. `robots_served` is what the page actually carries after WordPress core and the plugin reconcile it. For a plugin that does not serve the head, `rendered` holds its templates as the exporter rendered them (`rendered_by`, e.g. `bridge`): final text, with each variable it could not render taken out of the string and named in `unresolved`, and `template_source` saying whether each template was the page's own, its post type's or the plugin default. `rendered.open_graph` / `rendered.twitter` carry `image_width` / `image_height` for a local attachment, and `rendered.schema.graph` the schema nodes the plugin stored for the page (Rank Math), their variables rendered.
...
- **`integrations`** (`RawIntegration[]`) lists the outside services the site is connected to (`INTEGRATION_CATEGORIES`: analytics, crm, newsletter, ads, comments, captcha, cdn, other), each with its evidence. Evidence names a plugin, a setting, a script host, a form's wiring or an embed host, never a value. `reconnect_required` is `false` only for an embed, which has no account. `secret_present` says a credential is set on WordPress; the credential itself is never exported. `IntegrationReconnectRequiredIssue` (`code: 'integration_reconnect_required'`) is the one intake/handoff issue that lists every service to connect again.
...
| `ExecutionPlan` | An operation fully described before it runs: steps, union scope, risk, estimate, rollback, and the repository state it assumes. `ExecutionEstimate` meters cost before it is priced — `tokens`, `duration_ms`, `items`, `bytes_stored` (at rest) and `bytes_out` (moved); storage and egress are separate because they are billed by different rates against different quantities. |
