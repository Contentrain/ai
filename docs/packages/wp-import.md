...
**Unpublished content needs a credential.** Anonymous REST returns what WordPress shows the public: published posts and approved comments. With an Application Password, `fetchRestRawIR` also lists `future`, `draft`, `pending` and `private` posts (in the edit context, so password-protected posts are marked too, and [come in as drafts](#post-status)) and held comments. Trash is never imported. If the site refuses the credential those listings, the import falls back to the public one and says so in `warnings`. A credential the site rejects outright (401 on every route, as WordPress answers a wrong Application Password) is dropped after one check, and the whole import reads the public listings. Act on the result's `credential` rather than the warning text: `{ status: 'none' | 'accepted' | 'rejected', fell_back }`, where `fell_back` names each listing that did not use the credential (`posts`, `pages`, a custom type's REST base, `comments`, `comments:hold`). The WordPress status is kept as-is in `RawIR`, and [becomes entry metadata](#post-status) on conversion.
...
| any status with a post password (except `trash`) | `draft`, with `visibility: password` |

**Password-protected posts always come in as drafts.** Their body is the protected text, and nothing downstream (the query SDK, the Astro emitter) enforces `visibility: password`, so publishing one would put that text on a public site. `import-report.json` counts them in `password_protected_drafts`. The password itself is never read: `RawPost.password` holds the marker `[protected]`.
