...
- 383d7ae: `MigrateStudioClaim` gains an optional, signed `origin` — the migrated WordPress site as a bare origin (`https://host`; `http:` only for localhost). `validateMigrateStudioClaim` rejects anything that is not a canonical origin, so Studio can store it on the grant and fetch media from it alone.

### Patch Changes

- 64966a9: MigrateStudioClaim.origin: refuse a trailing-dot host; document that the origin must be exactly its canonical form (no userinfo, query or fragment; IDN in punycode).
...
- 897834b: `CapabilityManifest.access.achieved` is nullable: a site can answer nothing a rung describes
...
- 520f60f: Execution, approval and source-delta contracts — the shapes an operation is described, approved and recorded with
  ...
  - **`SourceDeltaPlan`** — the WordPress→repository delta, which is _not_
...
- 2fae9fb: Frontmatter values survive a round trip — and both readers agree on them
  ...
  **Both readers had the same bug, which is why this is one change and not two.**
  ...
  - A _string_ that would read back as another type is quoted; a real boolean,
...
- adf26c1: Component mounting and runtime binding contracts
...
- 487a061: The generic `api_key = …` secret rule now checks what it captured before it fires (#121).
