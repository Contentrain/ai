...
interface ActivityItem { tool: string; target: string; time: string }
...
const recentActivity = computed<ActivityItem[]>(() => {
  ...
  if (!op['tool']) return []
  return [{
    tool: op['tool'] as string,
    target: (op['target'] ?? '') as string,
    time: op['timestamp'] ? formatRelativeTime(op['timestamp'] as string) : 'recent',
  }]
})
        ...
        <section v-if="recentActivity.length > 0">
          ...
          <div class="space-y-2">
            <div v-for="(activity, i) in recentActivity" :key="i"
              ...
              <span class="ml-auto shrink-0 text-xs text-muted-foreground">{{ activity.time }}</span>
            </div>
          </div>
        </section>
