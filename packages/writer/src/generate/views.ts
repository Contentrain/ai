...
/** A field's definition, `cta.url` reaching into an object field. */
function fieldDef(ctx: Context, model: string, path: string): FieldDef | undefined {
  let def: FieldDef | undefined
  let fields = ctx.models.get(model)?.fields
...
class View {
  ...
  needs = new Set<'getEntry' | 'getCollection' | 'getStrings' | 'getSite' | 'getMenu' | 'siteConfig' | 'pages' | 'getPosts' | 'imageOf' | 'postHref' | 'pageHref' | 'termHref' | 'authorHref' | 'byId' | 'publicLinks' | 'publicHtml' | 'publicItems'>()
  lines: string[] = []
  body: string[] = []
  counter = 0
  ...
  /**
   * A field as the page may print it: a URL through the published set, links in rich text likewise, and
   * the same for a list's items. A link to a draft, a private page or the source site's address never
   * reaches the build. The value reads the entry optionally, so it can be computed before the entry is tested.
   */
  field(scope: Scope, path: string): string {
    const plain = `${scope.data}.${path}`
    const def = scope.model ? fieldDef(this.ctx, scope.model, path) : undefined
    const safe = `${scope.entry}?.data.${path}`
    if (def?.type === 'url') { this.needs.add('publicLinks'); return `publicHref(${safe})` }
    if (def?.type === 'richtext') { this.needs.add('publicHtml'); return `(await publicHtml(${safe}))` }
    const items = def?.type === 'array' && typeof def.items === 'object' ? def.items.fields ?? {} : {}
    const urls = Object.keys(items).filter(name => items[name]!.type === 'url')
    const html = Object.keys(items).filter(name => items[name]!.type === 'richtext')
    if (urls.length === 0 && html.length === 0) return plain
    this.needs.add('publicItems')
    return `(await publicItems(${safe}, [${urls.map(sq).join(', ')}], [${html.map(sq).join(', ')}]))`
  }
  ...
  value(value: PlanValue, scope: Scope | undefined): string {
    ...
    switch (kind) {
      case 'field': return this.field(need(), arg)
      case 'media': {
        const s = need()
  ...
  source(name: string, showTitle: boolean): string {
    ...
    const lib = (['authorHref', 'byId', 'getPosts', 'getSite', 'getStrings', 'imageOf', 'pageHref', 'postHref', 'termHref'] as const).filter(n => this.needs.has(n) || (n === 'byId' && this.needs.has('pages')))
    const links = (['getMenu', 'publicHtml', 'publicItems', 'publicLinks'] as const).filter(n => this.needs.has(n))
    const imports = [
      ...
      ...(links.length ? [`import { ${links.join(', ')} } from '../../lib/links'`] : []),
    ...
    const setup = [
      ...
      ...(this.needs.has('publicLinks') ? ['const publicHref = await publicLinks()'] : []),
    ]
    return `---
