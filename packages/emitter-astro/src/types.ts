...
export interface EmitOptions {
  ...
  seo?: boolean
  /**
   * Emit a sitemap and a `robots.txt` that points at it. Default true.
   *
   * The sitemap is generated by `@astrojs/sitemap` at build time rather than
   * computed here, because only the build knows the real set of URLs — every
   * dynamic route, every paginated page, whatever `getStaticPaths` returned. A
   * sitemap the emitter worked out on its own would be a second source of truth
   * that can disagree with the site it describes.
   *
   * Independent of `seo`: a producer that writes its own meta tags has not
   * thereby said it writes its own sitemap, and coupling the two would drop the
